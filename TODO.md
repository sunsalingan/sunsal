...
## 🚀 현재 버전: v1.2 (2026-01-08)
- **주요 변경**: 앱 설명서(User Guide) 고도화, 찜 목록(Wishlist) 점수 동기화.
- **상태**: 안정적 (Stable). 로컬/배포 환경 정상 작동 확인됨.
...
*   **기능 고도화**:
    ...
    *   **[New]** 프랜차이즈 별점 랭킹: 동일 브랜드 지점별 평점 비교 편차 기능 추가.
...
*   **유지보수**:
    ...
    *   **[Fixed]** 친구 찾기 모달 및 시스템 정상화 (2026-01-08)
    *   **[Improved]** 식당 상세 리뷰 중복 제거 및 날짜 포맷팅 (2026-01-08)
...
- **위치 기반 검색 정확도 (Location-based Search)**:
    - 증상: "롯데리아" 등 프랜차이즈 검색 시, 현재 위치(예: 문정동)의 가까운 지점(파크하비오점)이 결과에서 누락되는 현상.
    - 시도: `reverseGeocode`를 통해 "문정동"을 추출하여 "문정동 롯데리아"로 검색어 자동 보정을 시도했으나, API 결과가 불안정하여 보류(Pass)함.
    - 향후 계획: 네이버 Place API 파라미터 재분석 또는 클라이언트에 데이터 전체 로드 후 거리순 정렬 등 다른 방식 모색 필요.

## 📌 다음 작업 가이드
1.  **프랜차이즈 별점 랭킹**: 동일 브랜드(예: 스타벅스, 롯데리아)의 지점별 평점 편차를 비교하는 기능 구현.
2.  **모바일 최적화**: 배포된 웹앱의 모바일 사용성 점검.
3.  **검색 기능 재검토**: 현재 보류된 위치 기반 검색 로직 재설계.
4.  **배포**: 최신 변경사항 Firebase Hosting 배포.
